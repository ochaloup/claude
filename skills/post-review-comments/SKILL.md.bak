...
and so does a `python3 -c '…'` one-liner with the text inlined. Two steps every
time:

1. Write the comment text to `<scratch>/body-<n>.md` with the **Write tool** —
   no escaping, no shell.
2. Build the JSON with a one-liner that *reads* that file, so the command itself
   holds nothing but ASCII ids and paths.
...
### 7b. Add one thread per finding

Line or range:

```bash
python3 -c 'import json;b=open("<scratch>/body-1.md").read();open("<scratch>/thread-1.json","w").write(json.dumps({"query":"mutation($rev:ID!,$path:String!,$line:Int!,$body:String!){addPullRequestReviewThread(input:{pullRequestReviewId:$rev,path:$path,line:$line,side:RIGHT,body:$body}){thread{id}}}","variables":{"rev":"PRR_kwXXX","path":"src/fees.ts","line":214,"body":b}}))'
```

```bash
gh api graphql --input <scratch>/thread-1.json
```

Add `startLine:$start,startSide:RIGHT` to the input for a range. Use
`side:LEFT`/`startSide:LEFT` for removed lines. For a whole file, drop
`line`/`side` and pass `subjectType:FILE`.

One mutation per finding, one file per finding. Batching them into a single
document hides which one GitHub rejected, and a rejected anchor is the failure
