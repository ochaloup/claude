...
# SETUP
# 1. Using token from gh command. I cannot get it working properly when using new token from web
#    Such a token did not have enough permissions to work properly with pr review comments and get info from privat repositories
...
#    -> gh auth status
#    -> gh auth token
# 3. Copy the token and paste it into ~/.claude.json
#   "mcpServers": {
#     "github": {
#       "type": "stdio",
#       "command": "npx",
...
Split $ARGUMENTS by whitespace into tokens. Classify each:
...
Use the `$SAVE_PLAN_PATH` lookup conventions from `/save-plan` (resolve the path
via `python3 -c 'import os; print(os.environ.get("SAVE_PLAN_PATH",""))'`). Scan
for files matching this PR/branch, e.g. `*--<HEAD_BRANCH>--pr-<PR_NUMBER>--REVIEW*.md`
...
## 11. Persist the full report via /save-plan

Execute `/save-plan pr-<PR_NUMBER>` with the step-10 content as input.
`/save-plan` resolves the canonical filename (TYPE will be `REVIEW`) and writes
the file under `$SAVE_PLAN_PATH`. Do not write any file directly.
...
to the chat as the **last** output of the command. No prose after it except
