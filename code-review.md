Review branch changes against a base ref. Usage: /code-review [--base <ref>]
...
The command output happens in this strict order: **(1) summary in chat, (2)
save full report via `/save-plan`, (3) final table as the last step.**
...
### Step 2 — Persist the full report via `/save-plan`

Execute the `/save-plan` command (pass `pr-<N>` as context if a PR exists; no
...
**last** output of the command. No prose after it.
